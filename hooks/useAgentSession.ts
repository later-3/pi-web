...
import {
  ...
  deviceUnavailableErrorFromResponse,
...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const onConnectionFailure = opts.onConnectionFailure;
  ...
  const loadSession = useCallback(async (sid: string, showLoading = false, includeState = false) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw deviceUnavailableErrorFromResponse(res, errorBody) ?? new Error(`HTTP ${res.status}`);
      }
  ...
  const handleSend = useCallback(async (message: string, images?: AttachedImage[]) => {
    ...
    try {
      ...
      const definitivelyRejected = !promptRequestStarted
        || e instanceof DeviceUnavailableError
        || isPromptRejectedError(e);
      ...
      const availability = e instanceof DeviceUnavailableError
        ? await onConnectionFailure?.().catch(() => null)
        : null;
  ...
  }, [isNew, newSessionCwd, newSessionModel, session, ensureNewSession, ensureEventsConnected, promoteNewSession, waitForPromptSettlement, addNotice, cancelEventStreamGrace, closeEvents, composerDraftKey, onConnectionFailure, reconcileAgentState, restoreSubmission]);
