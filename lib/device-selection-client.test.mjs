...
test("initial confirmation absorbs one transient offline response", async () => {
  let probes = 0;
  await confirmSelectedGatewayDevice("mac-main", {
    attempts: 3,
    retryDelayMs: 0,
    fetchFn: async () => {
      probes += 1;
...
test("initial confirmation reports offline only after every attempt fails", async () => {
  let probes = 0;
  await assert.rejects(
    confirmSelectedGatewayDevice("linux-home", {
      attempts: 3,
      retryDelayMs: 0,
      fetchFn: async () => {
        probes += 1;
