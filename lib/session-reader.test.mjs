...
test("classifies only files inside the dedicated Chat session directory as managed", () => {
  assert.equal(isChatManagedSessionPath(join(getChatSessionsDir(), "chat-tool-1.jsonl")), true);
  assert.equal(isChatManagedSessionPath(join(getChatSessionsDir(), "..", "sessions", "personal.jsonl")), false);
  assert.equal(isChatManagedSessionPath(getChatSessionsDir()), false);
});

test("drops stale cached session paths after their files are removed", async () => {
  const root = mkdtempSync(join(tmpdir(), "pi-web-stale-session-"));
  const previousAgentDir = process.env.PI_CODING_AGENT_DIR;
  try {
    process.env.PI_CODING_AGENT_DIR = root;
    const missingPath = join(root, "sessions", "missing.jsonl");
    cacheSessionPath("missing-session", missingPath);
    invalidateSessionListCache();

    assert.equal(await resolveSessionPath("missing-session"), null);
    assert.equal(await resolveSessionIdByPath(missingPath), undefined);
  } finally {
...
test("lists Chat-managed sessions from the dedicated store as read-only", async () => {
  const root = mkdtempSync(join(tmpdir(), "pi-web-chat-sessions-"));
  const previousAgentDir = process.env.PI_CODING_AGENT_DIR;
  try {
    process.env.PI_CODING_AGENT_DIR = root;
    ...
    writeFileSync(
      join(chatDir, "2026-07-28T00-00-00Z_chat-tool-smoke.jsonl"),
      [
        JSON.stringify({
          type: "session",
          version: 3,
    ...
    const missingProject = join(root, "removed-project");
    writeFileSync(
      join(chatDir, "2026-07-28T00-00-01Z_chat-tool-missing-project.jsonl"),
      [
        JSON.stringify({
          type: "session",
          version: 3,
          id: "chat-tool-missing-project",
          timestamp: "2026-07-28T00:00:01.000Z",
          cwd: missingProject,
        }),
        "",
    ...
    const session = sessions.find((value) => value.id === "chat-tool-smoke");
    assert.equal(session?.sessionSource, "chat");
    assert.equal(session?.readOnly, true);
    assert.equal(session?.name, "Chat托管 · smoke");
    assert.equal(session?.projectAvailable, true);
    assert.equal(
      sessions.find((value) => value.id === "chat-tool-missing-project")?.projectAvailable,
      false,
    );
  } finally {
    if (previousAgentDir === undefined) delete process.env.PI_CODING_AGENT_DIR;
...
test("forced session listing bypasses the fresh server cache", async (t) => {
  ...
  // Each logical scan covers both the normal Pi store and the Chat evidence store.
...
test("a scan invalidated in flight retries before returning to its caller", async (t) => {
  ...
  // The invalidated logical scan retries both session stores.
  assert.equal(scans, 4);
});
