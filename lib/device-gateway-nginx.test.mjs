...
test("cloud gateway owns a useful page even when every backend is offline", () => {
  ...
  assert.match(config, /onclick="checkControl\(\)"/);
  assert.match(config, /fetch\("\/login",\{cache:"no-store"\}\)/);
  assert.match(config, /设备隧道仍不可达/);
  assert.doesNotMatch(config, /onclick="location\.reload\(\)"/);
