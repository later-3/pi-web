...
React DeviceWorkspaceRoot
       ├─ 启动只加载目录并乐观挂载，不做 health 探针
       ├─ 正常使用不轮询；真实操作失败后才补做 1 次 health 判断
       ├─ “重新检测”只执行 1 次；成功后重新挂载当前工作区
...
9. 执行面不做跨设备自动 failover。`/api/health` 的连接级 `502/504` 由网关转换成带设备 id 的结构化 `503 device_offline`；只有按需探针确认网关不可达后，UI 才暂时卸载该工作区，用户显式重新检测或选择另一台设备。
10. 全部成员离线时，云端 Nginx 直接返回无后端依赖的恢复页；不得把 Cloudflare 通用 502 当产品离线页。
11. 设备连通性采用需求驱动，不是后台状态机：冷启动与普通页面生命周期均不探测；正常发送直接执行，不增加预检延迟，只有发送或 SSE 建连失败后才补做 1 次 health 判断；设备切换和“重新检测”各做 1 次。只有结构化 `device_offline`、网关 `502/504` 或探针超时才进入“网关暂时无法连接”界面；应用自身普通 `503` 与浏览器自身断网不得冒充设备不可达。
12. “设备在线”不是准确文案：浏览器只能证明“云端网关当前能否到达该设备的 Pi Web relay”，不能证明机器电源或操作系统状态。UI 必须使用“暂时无法连接”，并明确设备本身可能仍在线。
...
hooks/useDeviceDirectory.ts   故障转移目录、按需单次探针、不可达状态与重试
...
| 远端健康 | 启动/空闲为 0 探针；发送失败、显式重检、设备切换时各 1 次 | 健康路径零额外延迟、零后台流量；确认网关不可达后才显示恢复页 |
| 设备切换 | POST + 目标单次探针各自有界；目标侧栏 ready 最多等待 6 秒 | 超时/错路由回滚原设备并显示错误；不重试、不逐台预连 |
...
- 启动和空闲期间没有 health fetch、interval、`online`/`visibilitychange` 探针；发送失败后只补做 1 次，普通应用 `503` 不误判；按钮与设备切换均只探测 1 次；
