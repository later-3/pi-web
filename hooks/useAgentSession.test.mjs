...
test("keeps the session event stream open through the idle grace window", () => {
  ...
  assert.match(sendSource, /const definitivelyRejected = !promptRequestStarted[\s\S]*?e instanceof DeviceUnavailableError[\s\S]*?isPromptRejectedError\(e\)/);
  assert.match(sendSource, /if \(!definitivelyRejected && sentSessionId\) \{[\s\S]*?waitForPromptSettlement[\s\S]*?return;/);
