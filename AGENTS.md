...
- Relay SSH preflight commands are long-lived enough to survive a network transition: `ConnectTimeout` covers only connection establishment and is insufficient. Keep application-level `ServerAliveInterval`/`ServerAliveCountMax` plus a local hard watchdog around the preflight, otherwise a half-open child can leave launchd reporting `running` forever while the cloud listener is absent.
