...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const handleSend = useCallback(async (message: string, images?: AttachedImage[]) => {
    ...
    try {
      ...
      if (promptRequestStarted && sentSessionId && !(e instanceof DeviceUnavailableError)) {
        void waitForPromptSettlement(sentSessionId, promptRunId);
        return;
      }
      rpcPromptPendingRef.current = false;
      ...
      // entered while the failed request was settling.
  ...
  }, [isNew, newSessionCwd, newSessionModel, session, ensureNewSession, ensureEventsConnected, promoteNewSession, waitForPromptSettlement, addNotice, cancelEventStreamGrace, closeEvents, opts.chatInputRef, onConnectionFailure]);
