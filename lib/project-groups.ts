...
export function getRecentProjects(sessions: readonly SessionInfo[]): RecentProject[] {
  const latestByProject = new Map<string, { root: string; modified: string; available: boolean }>();
  for (const session of sessions) {
    ...
    latestByProject.set(key, {
      root: !previous || session.modified > previous.modified ? root : previous.root,
      modified: !previous || session.modified > previous.modified ? session.modified : previous.modified,
      available: (previous?.available ?? false) || session.projectAvailable !== false,
    });
  }
...
export function getProjectAvailability(
  ...
  for (const session of sessions) {
    const key = workspaceKeyOf(session);
    if (!key) continue;
    availability.set(key, (availability.get(key) ?? false) || session.projectAvailable !== false);
  }
  return availability;
}
